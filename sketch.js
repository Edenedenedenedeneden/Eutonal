...
function setup() {
  ...
  totalRects =
    Math.ceil(width / rectSizeFill) * Math.ceil(height / rectSizeFill);
...
function drawRectangles(colorFilter) {
  if (started) {
    ...
    if (elapsedTime - lastRectTime > 42) {
      // Adjusted this value for faster drawing
      ...
      if (currentRectIndex < totalRects) {
        ...
        let color;
        if (random() < 0.5) {
          // 50% chance to be purple regardless of volume
          color = "#6B10C5";
        } else if (random() < 0.3) {
          // 30% chance to be green regardless of volume
          color = "#2BB516";
        } else {
...
function mousePressed() {
  if (allLoaded) {
    ...
    if (
      !started &&
      mouseX > startRectX &&
      mouseX < startRectX + startRectW &&
      mouseY > startRectY &&
      mouseY < startRectY + startRectH
    ) {
      started = true;
      startTime = millis();
      lastRectTime = 0;
    ...
    if (started) {
      ...
      if (
        mouseX > rect1X &&
        mouseX < rect1X + rectSizeW &&
        mouseY > rect1Y &&
        mouseY < rect1Y + rectSizeH
      ) {
      ...
      if (
        mouseX > rect2X &&
        mouseX < rect2X + rectSizeW &&
        mouseY > rect2Y &&
        mouseY < rect2Y + rectSizeH
      ) {
        // Change to the previous image in the array (cycling backward)
        changeImage(
          (currentImgIndex - 1 + otherTextImgs.length) % otherTextImgs.length
        );
...
function blinkArrow() {
  ...
  let alpha = map(
    sin((TWO_PI * (currentTime - arrowBlinkTime)) / blinkInterval),
    -1,
    1,
    50,
    255
...
function checkIfAllLoaded() {
  loadCount++;
  if (loadCount === otherTextImgs.length + otherTextSongs.length + 3) {
    // +3 for the background, '2024' and 'arrow' images
    allLoaded = true;
  }
}
